...
# Note on wallet generation:
"""
We do not save any information about wallets that get generated here.
"""
...
# 5. Group-Specific Settings

"""
You can create group-specific settings files named group_settings_<GROUP_ID>.json. These files allow you to override the default settings for specific groups. For example:

group_settings_123456789.json:
{
  "BUY_AMOUNT": 0.5,
  "SLIPPAGE": 10,
  "PRIORITY_FEE": 0.02,
  "AUTO_SELL": "True",
  "AUTO_SELL_HOLD_TIME": 30
}

This will use these settings for the group with ID 123456789, while other groups will use the default settings from buyconfig.json.
"""
...
- if there are two trades and you sell one then the one left will be sell 1
...
3. Fill in the `buyconfig.json` and `credentials.json` files as instructed.
